import { defineEventHandler, getHeader, getHeaders, getQuery, getRequestIP, readRawBody, setResponseStatus } from "h3";
...
function assertIpAllowed(event: any) {
  ...
  if (!requestIp || !allowedIps.includes(requestIp)) throw new Error("ip not allowed");
}
...
async function assertWebhookRateLimit(event: any, provider: string) {
  ...
  if (count > limit) {
    ...
    const error = new Error("rate limit exceeded") as Error & { retryAfterSec?: number };
...
function normalizeProviderCode(provider: string) {
  ...
  if (normalized !== "SCB" && normalized !== "KBANK") throw new Error("unsupported provider");
...
export default defineEventHandler(async (event) => {
  ...
  logEvent({ event: "webhook.inbound.received", data: { provider: providerCode, eventId, merchantId, requestIp, payloadSize: Buffer.byteLength(rawBody, "utf8") } });
  ...
  try {
    ...
    await verifyWebhookSignature({ rawBody, signature, timestamp, ...(merchantId ? { merchantId } : {}) });
  } catch (error: any) {
    const stored = await storeProviderCallback({
      providerCode,
      rawBody,
      body: (() => { try { return JSON.parse(rawBody || "{}"); } catch { return { _raw: rawBody, _invalidJson: true }; } })(),
      headers: getHeaders(event),
      queryParams: { ...(getQuery(event) as Record<string, unknown>), _meta: { eventId, merchantId, requestIp, provider: providerCode } },
      signatureValid: false,
      dedupeKey: `${providerCode}:EVENT:${eventId}`,
      callbackType: "GENERIC_WEBHOOK",
    });
    setEventRequestContext(event, { providerCallbackId: (stored as any).callbackId });
    setResponseStatus(event, 400);
    return { error: error?.message || "invalid webhook" };
  }

  const body = (() => { try { return JSON.parse(rawBody || "{}"); } catch { return { _raw: rawBody, _invalidJson: true }; } })();
  const stored = await storeProviderCallback({
    providerCode,
    rawBody,
    body,
