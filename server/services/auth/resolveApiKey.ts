...
export async function resolveApiKey(fullApiKey: string): Promise<AuthContext> {
  ...
  if (!parsed) throw new AppError("UNAUTHORIZED", "Malformed API key", 401);
  ...
  const record = await prisma.apiKey.findFirst({
    ...
    include: { tenant: true, merchantAccount: true },
  });

  if (!record) throw new AppError("UNAUTHORIZED", "Invalid API key", 401);
  if (!verifyApiKeySecret(parsed.secret, record.secretHash)) {
    throw new AppError("UNAUTHORIZED", "Invalid API key", 401);
  }
  if (record.expiresAt && record.expiresAt.getTime() < Date.now()) {
    await prisma.apiKey.update({ where: { id: record.id }, data: { status: "EXPIRED" } }).catch(() => undefined)
    throw new AppError("UNAUTHORIZED", "API key expired", 401);
  }
  if (record.tenant.status !== "ACTIVE") throw new AppError("FORBIDDEN", "Tenant is inactive", 403);
