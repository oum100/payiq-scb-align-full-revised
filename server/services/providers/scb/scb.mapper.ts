...
export function mapScbCreateResponse(args: {
  ...
  const redirectUrl =
    args.response?.data?.data?.url || args.response?.redirectUrl || null;
  ...
  const responseData = args.response?.data as
  ...
  const providerTransactionId =
    (typeof nestedData?.transactionId === "string"
  ...
  const providerReference =
    args.response?.data?.request?.ref1 || args.request.ref1;

  const providerQrRef =
    args.response?.data?.request?.ref3 || args.request.ref3 || null;
...
export function mapScbInquiryResponse(args: {
  ...
  const providerReference =
    item?.billPaymentRef1 || args.request.reference1 || null;

  const providerQrRef = item?.billPaymentRef3 || null;

  const externalStatus =
    item?.status || item?.paymentStatus || item?.statusText || undefined;
  ...
  return {
    ...
    status: mapScbStatusToInternal(
      externalStatus ?? (item ? "SUCCESS" : "FAILED"),
    ),
    rawResponse: args.rawResponse,
  };
}
