...
export async function deliverWebhook(webhookDeliveryId: string) {
  ...
  try {
    ...
    const updated = await prisma.webhookDelivery.update({
      ...
      data: {
        status:
          delivery.attemptNumber + 1 >= delivery.webhookEndpoint.maxAttempts
            ? "DEAD"
            : "RETRYING",
        responseStatusCode: res.status,
        responseBody: text,
        ...
        nextAttemptAt:
          delivery.attemptNumber + 1 >= delivery.webhookEndpoint.maxAttempts
            ? null
            : computeNextAttempt(delivery.attemptNumber + 1),
      },
    })
    ...
    if (updated.status === "RETRYING") {
      await webhookQueue.add(
        ...
        { webhookDeliveryId: delivery.id },
        {
          delay: Math.max(
            1000,
            new Date(updated.nextAttemptAt!).getTime() - Date.now(),
          ),
