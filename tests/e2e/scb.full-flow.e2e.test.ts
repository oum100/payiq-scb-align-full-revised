...
describe("SCB full flow e2e-lite", () => {
  ...
  test("create payment -> callback ingress -> callback process -> payment succeeded", async () => {
    ...
    const rawCallbackBody = JSON.stringify({
      ...
      billPaymentRef3: "PYIQPIFF001PIQFF001",
    ...
    expect(queueJobs[0]?.providerCallbackId).toBe("pcb_ff_001");

    await processProviderCallback("pcb_ff_001");
    ...
    expect(updatedPayment?.providerTransactionId).toBe("scb-ff-txn-001");
    ...
    expect(webhookJobs).toEqual(
      expect.arrayContaining([
        { paymentIntentId: "pi_ff_001", eventType: "PAYMENT_SUCCEEDED" },
      ]),
    );
  });
});
