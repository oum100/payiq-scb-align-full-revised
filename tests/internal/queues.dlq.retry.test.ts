...
describe("POST /api/internal/queues/dlq/retry", () => {
  beforeEach(() => {
    ...
    providerCallbackFindUniqueMock.mockReset();
    ...
    providerCallbackFindUniqueMock.mockResolvedValue({
      id: "wh_123",
      tenantId: "tenant_1",
      providerCode: "SCB",
    });
  ...
  it("requeues webhookInbound DLQ jobs using queue policy", async () => {
      ...
      jobs: [
        {
          ...
          payload: { providerCallbackId: "evt_123" },
        },
      ],
    });
    ...
    expect(providerCallbackFindUniqueMock).toHaveBeenCalledTimes(1);
